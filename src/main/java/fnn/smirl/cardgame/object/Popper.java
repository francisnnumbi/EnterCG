...
public class Popper extends GObject 
implements Alignment, Padding, Stroke, Text, Rectangle, Shadow {

 private Paint paintH, paint, paint2, paint3;
 ...
 private String header = "", message = "", btnText = "", baseText = "";
 private int shadowColor = Color.GREEN;
 private float shadowRadius = 0;
 private PointF shadowPoint = new PointF(0, 0);

 public Popper(Context ctx, PointF parentSize) {
	...
	paintH.setTextSize(paint.getTextSize() * 1.5f);
 ...
 public void setText(String header, String msg, String btnText) {
	this.header = header;
	message = msg;
	this.btnText = btnText;
	String lt = msg.length() > header.length() ? msg : header;
	lt = lt.length() > btnText.length() ? lt : btnText;
	tempRect.set(tempRect.left, tempRect.top, tempRect.left + paint.measureText(lt),
							 tempRect.top + paint.getTextSize());
 ...
 public void organize() {
	String lt = message.length() > header.length() ? message : header;
	lt = lt.length() > btnText.length() ? lt : btnText;
	float f = (paint.measureText(baseText) - paint.measureText(lt)) / 2;
	tempRect = new RectF(tempRect.left, tempRect.top,
											 tempRect.left + paint.measureText(lt), tempRect.top + paint.getTextSize());
	rect = new RectF(tempRect.left - f - padLeft,
									 tempRect.top - 3 - paintH.getTextSize() - padTop, tempRect.right + f + scale + padRight,
									 tempRect.bottom + (paint.getFontMetrics().descent * scale) + 10 + paint.getTextSize() + padBottom);
 }
 ...
 public void setHeaderTextPaint(Paint paint) {
	this.paintH = paint;
 }
 ...
 public void setTextStyle(TextStyle style) {
	...
	paintH.setTypeface(paint.getTypeface());
 }
 ...
 public void onDraw(Canvas canvas) {
	...
	paintH.setShadowLayer(shadowRadius, shadowPoint.x, shadowPoint.y, shadowColor);
	if (rectVisible) {
	 if (isRoundRectangle) {
	...
	canvas.drawText(header, tempRect.left + ((tempRect.width() - paintH.measureText(header)) / 2), (tempRect.top + paint.getTextSize() - paintH.getTextSize() - 3), paintH);
	canvas.drawLine(tempRect.left, tempRect.top - 3, tempRect.right, tempRect.top - 3, paintH);
	canvas.drawText(message, tempRect.left, (tempRect.top + paint.getTextSize()), paint);
	GameView.blinkRate++;
	if (!GameView.blink) {
	 canvas.drawText(btnText, tempRect.left + ((tempRect.width() - paint.measureText(btnText)) / 2), (tempRect.top + paint.getTextSize() + paint.getTextSize() + (paint.getFontMetrics().descent * scale) + 10), paint);
	}
	if (GameView.blinkRate >= 20) {
	 GameView.blink = !GameView.blink;
	 GameView.blinkRate = 0;
	}
